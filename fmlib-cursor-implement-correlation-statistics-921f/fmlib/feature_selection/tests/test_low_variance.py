...
def test_statistics_stage_registers_low_variance_before_correlation() -> None:
    frame = pd.DataFrame({"feature": [1.0, 2.0], "response": [0, 1]})
    ...
    context = _context(frame, ["feature"], config=config)

    methods = [selector.method_name for selector in StatisticsStage().build_selectors(context)]

    assert methods == ["low_variance", "correlation"]
