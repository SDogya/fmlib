...
def test_drops_constant_and_quasi_constant() -> None:
    fs_config = FeatureSelectionConfig.from_dict(
        {
            "statistics": {
                "constants": {"enabled": True, "max_frequency": 0.9, "min_unique": 2},
...
def test_min_unique_rule() -> None:
    config = ConstantsConfig(enabled=True, max_frequency=1.0, min_unique=5)
    selector = ConstantsSelector(config)
    fs_config = FeatureSelectionConfig.from_dict(
        {
            "statistics": {
                "constants": {"enabled": True, "max_frequency": 1.0, "min_unique": 5},
                "null_rate": {"enabled": False},
                "correlation": {"enabled": False},
            },
        },
...
def test_keeps_all_when_thresholds_relaxed() -> None:
    config = ConstantsConfig(enabled=True, max_frequency=1.0, min_unique=None)
    selector = ConstantsSelector(config)
...
def test_all_null_column_is_left_to_null_rate_selector() -> None:
    frame = _frame()
    frame["all_null"] = None
    config = ConstantsConfig(enabled=True, max_frequency=0.5)
    fs_config = FeatureSelectionConfig()
...
def test_spark_rejects_map_type_before_collect() -> None:
    ...
    try:
        ...
        config = FeatureSelectionConfig.from_dict(
            {
                "statistics": {
                    "constants": {"enabled": True, "max_frequency": 0.99},
                    "null_rate": {"enabled": False},
                    "correlation": {"enabled": False},
                },
            },
