...
from fmlib.feature_selection.base import FeatureDecision, StageContext, derive_seed
...
class CorrelationSelector:
    ...
    Algorithm follows ``draft/correlation.py``:

    1. take ``candidates ∩ schema.continuous`` (not Spark dtype discovery);
    2. sample about ``min(config.max_rows, execution.max_local_rows)`` train rows
       with a derived seed (Spark ``sample``, not ``limit``, to keep partitions);
    ...
    def select(
        ...
        seed = derive_seed(context.seed, self.stage_name, self.method_name)
        train = context.datasets["train"]
        if _is_spark_dataframe(train):
            corr_matrix, null_rates, evaluated = self._compute_stats_spark(
                train,
                columns,
                max_rows=max_rows,
                seed=seed,
            ...
            corr_matrix, null_rates, evaluated = self._compute_stats_pandas(
                train,
                columns,
                max_rows=max_rows,
                seed=seed,
    ...
    def _compute_stats_spark(
        ...
        *,
        ...
        Mirrors the draft pipeline: filter all-null columns, median-impute,
        assemble a feature vector, then call ``Correlation.corr``. Uses
        ``sample`` (not ``limit``) so partitions stay distributed.
        """
        try:
        ...
        sampled = self._sample_spark(projected, max_rows=max_rows, seed=seed).cache()
        ...
        try:
            ...
            summary = sampled.agg(*aggregations).collect()[0]
            ...
            working = sampled.select(*evaluated_aliases)
            filled = (
                Imputer(
                    strategy="median",
            ...
            try:
                ...
                msg = (
                    ...
                    f"on a sample targeting {max_rows} rows. Root cause: {_root_cause(exc)}."
                )
    ...
    @staticmethod
    def _sample_spark(frame: Any, *, max_rows: int, seed: int) -> Any:
        """Sample approximately ``max_rows`` rows while preserving partitions."""
        ...
        if fraction >= 1.0:
            return frame
        return frame.sample(withReplacement=False, fraction=fraction, seed=seed)
    ...
    def _compute_stats_pandas(
        ...
        *,
        ...
        if projected.empty:
            return np.empty((0, 0)), dict.fromkeys(columns, 0.0), []
        sample_size = min(max_rows, len(projected))
        bounded = projected if sample_size >= len(projected) else projected.sample(n=sample_size, random_state=seed)
    ...
    def _resolve_drops(
        ...
        Matches the draft upper-triangle scan: already-dropped features are skipped
        so each feature yields at most one drop decision.
        """
