...
class ConstantsSelector:
    ...
    def _compute_stats_spark(
        ...
        # Stable aliases avoid dots/spaces in column names breaking agg output fields.
        alias_by_col = {col: f"c{i}" for i, col in enumerate(columns)}
        projected = train.select(*[_quoted_col(col).alias(alias_by_col[col]) for col in columns])
        ...
        try:
            ...
            for chunk in chunks:
                mode_aggs = [
                    F.mode(F.col(alias_by_col[col])).alias(f"{alias_by_col[col]}__mode")
                    for col in chunk
                ]
                mode_row = projected.agg(*mode_aggs).collect()[0]
                for col in chunk:
                    alias = alias_by_col[col]
                    modes[col] = mode_row[f"{alias}__mode"]
            ...
            for chunk in chunks:
                ...
                for col in chunk:
                    ...
                    if self.config.min_unique is not None:
                        summary_aggs.append(F.countDistinct(value).alias(f"{alias}__nunique"))
                    summary_aggs.append(F.count(value).alias(f"{alias}__non_null"))
                    if mode is None:
                        summary_aggs.append(F.lit(0).alias(f"{alias}__mode_count"))
                    elif _is_nan(mode):
                        summary_aggs.append(F.sum(F.isnan(value).cast("long")).alias(f"{alias}__mode_count"))
                    else:
                        summary_aggs.append(
                            F.sum((value == F.lit(mode)).cast("long")).alias(f"{alias}__mode_count"),
                        )
                summary = projected.agg(*summary_aggs).collect()[0]
                for col in chunk:
                    alias = alias_by_col[col]
                    non_null = int(summary[f"{alias}__non_null"] or 0)
                    if non_null == 0:
                        result[col] = (0, 0.0)
                        continue
                    mode_count = int(summary[f"{alias}__mode_count"] or 0)
                    if self.config.min_unique is None:
                        n_unique = 1 if mode_count == non_null else 2
                    else:
                        n_unique = int(summary[f"{alias}__nunique"] or 0)
    ...
    def _decide_feature(
        ...
        *,
        ...
        if n_unique == 0:
            # The draft constants algorithm leaves all-null columns to null-rate filtering.
            return None
        if n_unique <= 1:
