...
class FeatureTransformer(torch.nn.Module):
    ...
    def __init__(
        ...
        late_fusion_layer_norm: Optional[Union[torch.nn.Module, List[torch.nn.Module]]] = None,
        ...
        Args:
            ...
            late_fusion_layer_norm (Optional[Union[torch.nn.Module, List[torch.nn.Module]]]):
                Один слой нормализации или список слоёв для позднего слияния скрытых состояний.
                При передаче списка в `forward` необходимо передавать список `hidden_states` той же длины.
        ...
        if isinstance(late_fusion_layer_norm, list):
            self.late_fusion_layer_norm: Optional[torch.nn.Module] = None
            self.late_fusion_layer_norms: Optional[torch.nn.ModuleList] = torch.nn.ModuleList(late_fusion_layer_norm)
        else:
            self.late_fusion_layer_norm = late_fusion_layer_norm
            self.late_fusion_layer_norms = None
    ...
    def forward(
        ...
        **kwargs,
        ...
        Args:
            ...
            hidden_states (Optional[Union[torch.Tensor, List[torch.Tensor]]]): Один тензор или список
                тензоров скрытых состояний. При использовании нескольких `late_fusion_layer_norm`
                необходимо передавать список той же длины.
        ...
        Raises:
            ...
            ValueError: Если длина списка `hidden_states` не совпадает с числом `late_fusion_layer_norm`.
        """
        ...
        # late fusion branch: concat aggregated states with (possibly multiple) hidden_states
        if self.late_fusion_layer_norm is not None or self.late_fusion_layer_norms is not None:
            ...
            late_norms: List[torch.nn.Module] = (
                list(self.late_fusion_layer_norms)
                if self.late_fusion_layer_norms is not None
                else [self.late_fusion_layer_norm]
            )
            if len(hidden_states_list) != len(late_norms):
                raise ValueError(
                    f"Number of hidden_states ({len(hidden_states_list)}) must match "
                    f"number of late_fusion_layer_norms ({len(late_norms)})"
                )
            for layer_norm, hs in zip(late_norms, hidden_states_list):
                aggregated_states = torch.cat([aggregated_states, layer_norm(hs)], dim=-1)
    ...
    def forward(
        ...
        **kwargs,
        ...
        Args:
            ...
            hidden_states (Union[torch.Tensor, List[torch.Tensor]]): Один тензор или список
                тензоров скрытых состояний.
