...
class UniformBatching:
    def __init__(self: Self, length: int, batch_size: int) -> None:
        length, batch_size = validate_input(length, batch_size)
    ...
    @property
    def batch_count(self: Self) -> int:
        return uniform_batch_count(self.length, self.batch_size)

    def __len__(self: Self) -> int:
        return self.batch_count

    def get_limits(self: Self, index: int) -> Tuple[int, int]:
    ...
    def __getitem__(self: Self, index: int) -> Tuple[int, int]:
