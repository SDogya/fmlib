...
def validate_num_replicas(num_replicas: int) -> int:
    if num_replicas < 1:
        msg: str = f"Num Replicas is invalid. Got {num_replicas}."
        raise ValueError(msg)
    return num_replicas
...
def validate_curr_replica(curr_replica: int, num_replicas: int) -> int:
    ...
    if (curr_replica < 0) or (num_replicas <= curr_replica):
        msg: str = f"Curr Replicas is invalid. Got {curr_replica}."
        raise ValueError(msg)
    return curr_replica
...
class Partitioning:
    ...
    def generate_raw_indices(self: Self, length: int) -> torch.LongTensor:
    ...
    def replica_indices(self: Self, raw_indices: torch.LongTensor) -> torch.LongTensor:
    ...
    def generate(self: Self, length: int) -> torch.LongTensor:
    ...
    def __call__(self: Self, length: int) -> torch.LongTensor:
