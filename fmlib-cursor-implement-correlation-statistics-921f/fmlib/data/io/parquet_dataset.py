import warnings
from typing import Callable, Iterator, List, Optional, Self, Union, cast
...
class ParquetDataset(IterableDataset):
    ...
    Аргументы:
        ...
        make_mask_name (Callable[[str], str], optional): Функция для генерации имени маски. По умолчанию DEFAULT_MAKE_MASK_NAME.
        ...
        pyarrow_to_batches_kwargs (Dict[str, Any], optional): Дополнительные параметры для метода to_batches в PyArrow Dataset.
    ...
    def compute_length(self: Self) -> int:
        ...
        if num_replicas not in self.cached_lengths:
            if len(self.cached_lengths) > 0:
                msg: str = "`num_replicas` changed. Unable to reuse cached length."
                warnings.warn(msg, stacklevel=2)
    ...
    def __len__(self: Self) -> int:
        ...
        msg: str = "This instance doesn't support `len()` method. You can enable it by setting `do_compute_length=True`."
        raise TypeError(msg)
