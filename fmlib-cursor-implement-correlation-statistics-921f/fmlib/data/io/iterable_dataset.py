from typing import Dict, Iterator, Optional, Self

import torch
import torch.utils.data as data

from fmlib.constants.io import DEFAULT_REPLICAS_INFO
from fmlib.data.utils.batching import UniformBatching, uniform_batch_count

from .implementation.named_columns import NamedColumns
from .partitioning.partitioning import Partitioning, partitioning_per_replica
from .partitioning.replicas import ReplicasInfoProtocol
...
def validate_batch_size(batch_size: int) -> int:
    if batch_size <= 0:
        msg: str = "`batch_size` must be a positive integer."
...
class IterableDataset(data.IterableDataset):
    ...
    def get_indices(self: Self) -> torch.LongTensor:
        """
    ...
    def get_batching(self: Self) -> UniformBatching:
        """
    ...
    def __iter__(self: Self) -> Iterator[Batch]:
        """
