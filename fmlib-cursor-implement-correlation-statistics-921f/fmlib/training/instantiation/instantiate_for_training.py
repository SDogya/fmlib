...
from fmlib.utils import flatten_dict
from fmlib.constants.ignore_keys import DEFAULT_IGNORE_KEYS
...
def instantiate_accelerator(cfg: DictConfig) -> accelerate.Accelerator:
    ...
    if "tracker" in cfg.accelerator:
        tracker = hydra.utils.instantiate(cfg.accelerator.tracker)
        del cfg.accelerator.tracker
    else:
        tracker = None
    ...
    if tracker is not None:
        if "log_params" in cfg:
            log_params = flatten_dict(
                {param: cfg.get(param, None) for param in cfg.log_params},
                ignore_keys=DEFAULT_IGNORE_KEYS,
            )
        else:
            log_params = None
        accelerator.init_trackers(tracker.run_name, config=log_params)
    return accelerator
...
def log_static_training_params(pipeline: torch.nn.Module, accelerator: accelerate.Accelerator) -> None:
    """
    Логирует статичные параметры обучения: число процессов, параметров модели и шагов аккумуляции.
    """
    total_params: int = sum(p.numel() for p in pipeline.parameters())
    static_params = {
        "num_processes": accelerator.num_processes,
        "total_number_model_parameters": total_params,
        "gradient_accumulation_steps": accelerator.gradient_accumulation_steps,
    }
    accelerator.print(static_params)
    accelerator.log(static_params, step=0)
...
def instantiate_state(cfg: DictConfig) -> TrainingState:
    ...
    log_static_training_params(pipeline, accelerator)
