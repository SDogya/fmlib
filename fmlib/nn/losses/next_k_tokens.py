...
class NextKTokensLoss(torch.nn.Module):
    ...
    def __init__(
        ...
        if feature_loss_weights is not None:
            assert all(key in self.model_columns_meta for key in feature_loss_weights)
    ...
    def create_labels_with_horizon(
        ...
        shifted_labels = labels[:, horizon_offset:].contiguous()  # shuft labels
    ...
    def calculate_delta_loss(
        self: Self, logits: torch.Tensor, labels: torch.Tensor, horizon_offset: int, padding_mask: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        shifted_logits, shifted_labels = self.create_labels_with_horizon(
            input_ids=self.get_timedeltas(labels, horizon_offset, padding_mask),
            logits=logits,
    ...
    def forward(self: Self, outputs: Batch, targets: Batch, **kwargs) -> torch.Tensor:
        ...
        self.stats = {key: value.detach().cpu().item() for key, value in losses.items()}
        all_losses = torch.cat([v.reshape(-1) for v in losses.values()])
