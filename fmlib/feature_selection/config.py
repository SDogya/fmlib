...
class StatisticsCacheConfig:
    ...
    Метрики вычисляются для кандидатов, оставшихся перед текущим шагом, и
    сохраняются по методу, snapshot данных, параметрам вычисления и упорядоченному
    списку кандидатов. Пороги применяются позже и не входят в ключ кеша.
