...
def prepare_numeric_frame(
    ...
    *,
    ...
    Пропуски в числовых данных остаются NaN для нативной обработки в LightGBM.
    Если модели нужна импутация (RF в BorutaSHAP), её выполняет сам селектор,
    не изменяя общую выборку. Выборка стратифицируется по целевой переменной, если
