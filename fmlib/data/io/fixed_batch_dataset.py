import warnings
from typing import Callable, Iterator, List, Optional, Protocol, Self, Tuple, cast

import torch
from torch.utils.data import IterableDataset

from fmlib.constants.batches import GeneralBatch, GeneralCollateFn
from fmlib.constants.io import DEFAULT_COLLATE_FN
    ...
    for key, value in batch.items():
        ...
        if strict:
            if batch_size != new_batch_size:
                msg: str = f"Batch size mismatch {key}: {batch_size} != {new_batch_size}"
...
class FixedBatchSizeDataset(IterableDataset):
    ...
    def __init__(
        ...
        if int_batch_size < 1:
            msg: str = f"Insufficient batch size. Got {int_batch_size=}"
            raise ValueError(msg)
    ...
    def get_batch_size(self: Self, batch: GeneralBatch) -> int:
