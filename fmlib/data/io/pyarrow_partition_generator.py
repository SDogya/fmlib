...
def shuffle_dataset_fragments(dataset: da.Dataset, generator: torch.Generator | None = None) -> da.Dataset:
    ...
    new_dataset: da.Dataset = da.dataset(
...
def shuffle_dataset(shufflable: ShufflableDataset) -> da.Dataset:
    """Опционально перемешивает датасет, если определен генератор."""
    if shufflable.generator is None:
        dataset: da.Dataset = shufflable.dataset
    else:
        dataset: da.Dataset = shuffle_dataset_fragments(
...
class FragmentIterator:
    ...
    Аргументы:
        ...
        dataset (da.Dataset): PyArrow-датасет, поддерживающий метод `get_fragments`.
    ...
    def __iter__(self: Self) -> Iterator[NamedColumns]:
        fragment: da.Fragment
        dataset: da.Dataset = shuffle_dataset(self)
            ...
            **self.pyarrow_kwargs,
            ...
            yield NamedColumns(
                columns={
                    ...
                    **to_sequence_columns(batch, self.metadata, self.device),
                },
                make_mask_name=self.make_mask_name,
            )
