...
from fmlib.constants.metadata import PADDING_FLAG, SEQUENCE_LENGTH_FLAG, SEQUENTIAL_FLAG
from fmlib.data.io.implementation.indexing import get_offsets
from fmlib.data.io.metadata import Metadata
from fmlib.data.utils.typing.dtype import torch_to_pyarrow
...
class BatchGenerator:
    ...
    def generate_lengths(self: Self) -> torch.LongTensor:
    ...
    def generate_phases(self: Self) -> torch.FloatTensor:
        return MAX_PHASE * torch.rand(self.batch_size, dtype=torch.float32, generator=self.generator)

    def generate_frequencies(self: Self) -> torch.FloatTensor:
        ones: torch.Tensor = torch.ones(self.batch_size, dtype=torch.float32)
    ...
    def generate_xs(self: Self) -> Batch:
    ...
    def generate_base(self: Self) -> Batch:
    ...
    def sin(self: Self, base: Batch) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def cos(self: Self, base: Batch) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def generate_batch(self: Self) -> Batch:
    ...
    def sin_padded(self: Self, batch: Batch) -> Batch:
    ...
    def cos_padded(self: Self, batch: Batch) -> Batch:
    ...
    def to_padded(self: Self, batch: Batch) -> Batch:
    ...
    def generate_padded(self: Self) -> Batch:
...
def make_metadata(batch_generator: BatchGenerator) -> Metadata:
    return {
        ...
        SIN_X: {
            ...
            SEQUENCE_LENGTH_FLAG: batch_generator.max_length,
        },
        SIN_SIN: {
            SEQUENTIAL_FLAG: True,
            PADDING_FLAG: PADDING,
