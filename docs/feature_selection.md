...
Для LightGBM в BorutaSHAP также переносится `best_iteration_` лучшего Optuna
trial: он заменяет исходный потолок деревьев при повторных обучениях внутри
Boruta, где отдельного `eval_set` для early stopping уже нет.
